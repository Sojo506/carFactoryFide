...
public class AssemblyPanel extends javax.swing.JPanel {
    ...
    public void onAddMaterialToLine(int lineIndex) {
        if (selectedMaterial != null) {
            ...
            if (agregado) {
                // Actualiza los materiales y el fondo de la línea específica
                updateAssemblyLineMaterials(lineIndex);
                updateAssemblyLineBackgrounds();

                // Refresca la cinta visualmente, SOLO el botón afectado
                conveyorBeltPanel.removeMaterialFromBelt(selectedMaterial);

                // Verifica si el auto ya se completó
                AssemblyLine line = controller.getAssemblyLines().getElement(lineIndex);
                ...
                if (line.getOrder() != null && line.getOrder().getCar().isComplete()) {
                    ...
                    // Limpiar SOLO la lista de la línea completada
                    switch (lineIndex) {
                        case 0 ->
                            assemblyList0.setListData(new String[0]);
                        case 1 ->
                ...
                // Siempre limpia el estado de selección
            ...
            // Restablece visualmente la cinta y el trashcan
            if (mainView != null) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (line != null && line.isOccupied() && line.getOrder() != null) {
                if (line.getOrder().getStatus() == model.enums.OrderStatus.IN_PROGRESS) {
                    ...
                    /*} else if (line.getOrder().getStatus() == model.enums.OrderStatus.COMPLETED) {
                    bgColor = new Color(255, 215, 0); // Dorado completada*/
                } else {
                    bgColor = Colors.INACTIVE_LINE; // Tu color default activo
                }
            } else {
                bgColor = Colors.INACTIVE_LINE; // Color por defecto gris
            }
